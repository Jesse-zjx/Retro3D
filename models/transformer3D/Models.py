...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        # self.schnet = SchNet(energy_and_force=False, cutoff=10.0, num_layers=6, hidden_channels=d_inner, 
        #                      num_filters=d_inner, num_gaussians=50, out_channels=d_model)
        self.comenet = ComENet(hidden_channels=d_inner, middle_channels=d_k, out_channels=d_model)
    ...
    def forward(self, src_seq, src_mask, src_pos, src_z, src_index, src_atoms, return_attns=False):
        ...
        # enc_output_pos = self.schnet(src_pos, src_z, src_index)
        enc_output_pos = self.comenet(src_pos, src_z, src_index)

        pos_bias = torch.zeros_like(enc_output)

        for i in range(pos_bias.shape[0]):
            feature = enc_output_pos[src_index==i]
            index = src_atoms[src_index==i]
            pos_bias[i, index] = feature

        enc_output = self.rate1 * enc_output + self.rate2 * pos_bias
        
        # enc_output_pos = enc_output_pos.unsqueeze(1)
        
        # enc_output = torch.cat([enc_output_pos,enc_output],dim=1)
        enc_output = self.dropout(enc_output)
        enc_output = self.layer_norm(enc_output)

        # src_pos_mask = torch.ones(enc_output_pos.shape[0], 1, enc_output_pos.shape[1])
        # src_pos_mask = src_pos_mask.eq(1)
        # src_pos_mask = src_pos_mask.cuda()

        # src_mask = torch.cat([src_pos_mask,src_mask],dim=-1)
