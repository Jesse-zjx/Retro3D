...
from utils.smiles_utils import get_context_alignment, get_nonreactive_mask, \
                                smi_tokenizer, remove_am_without_canonical, re_atommap
...
class USPTO_MIT(Dataset):
    def __init__(self, config, mode, rank=-1):
        ...
        if self.vocab_file in os.listdir(self.root):
            ...
            train_data = open(os.path.join(self.root, 'train.txt'), "r").readlines()
            val_data = open(os.path.join(self.root, 'valid.txt'), "r").readlines()
            raw_data = train_data + val_data
            self.build_vocab_from_raw_data(raw_data)

        self.data = open(os.path.join(self.root, '{}.txt'.format(mode)), 'r').readlines()
    ...
    def build_processed_data(self, raw_data):
        ...
        for i in range(len(reactions)):
            ...
            multi_data.append({"reacts":r, "prod":p, "class":'<UNK>'})
    ...
    def parse_smi(self, prod, reacts, react_class, build_vocab=False, randomize=False):
        ...
        prod, reacts = re_atommap(prod, reacts)
